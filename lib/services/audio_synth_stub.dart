...
void playXylophoneNote(int noteIndex) {
  ...
  _getOrCreateWavFile('xylo_$noteIndex', 0.6, (data, sampleRate) {
...
void playAnimalSound(String animalEmoji) {
  if (animalEmoji == '🐱') {
    // Cat Meow
    _getOrCreateWavFile('cat_meow', 0.45, (data, sampleRate) {
      for (int i = 0; i < data.length; i++) {
        final t = i / sampleRate;
    ...
    // Dog Bark
    _getOrCreateWavFile('dog_bark', 0.35, (data, sampleRate) {
      for (int i = 0; i < data.length; i++) {
        final t = i / sampleRate;
        double sampleValue = 0.0;
        double decay = 0.0;
    ...
    // Duck Quack
    _getOrCreateWavFile('duck_quack', 0.2, (data, sampleRate) {
      for (int i = 0; i < data.length; i++) {
        final t = i / sampleRate;
    ...
    // Frog Ribbit
    _getOrCreateWavFile('frog_ribbit', 0.2, (data, sampleRate) {
      for (int i = 0; i < data.length; i++) {
        final t = i / sampleRate;
    ...
    // Cow Moo
    _getOrCreateWavFile('cow_moo', 0.85, (data, sampleRate) {
      for (int i = 0; i < data.length; i++) {
        final t = i / sampleRate;
...
void playSparkleSound() {
  _getOrCreateWavFile('sparkle', 0.25, (data, sampleRate) {
    for (int i = 0; i < data.length; i++) {
      final t = i / sampleRate;
      double sampleValue = 0.0;
...
void playRaindropSound() {
  _getOrCreateWavFile('raindrop', 0.08, (data, sampleRate) {
    for (int i = 0; i < data.length; i++) {
      final t = i / sampleRate;
